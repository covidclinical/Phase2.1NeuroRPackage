...
devtools::install_github("jackwasey/icd@e54bb24")
library(icd)
...
setwd("/FourCePhase2.1Neuro") # set to the main folder of the zip-file sent by M.Hutch

# 4CE SITEID - MAKE SURE UPPERCASE
currSiteId = "NWU"

# 4CE PHASE1.1 AND 2.1 DATA FILES DIRECTORY
data_dir = '/share/fsmresfiles/mrh1996/4ceData_files_20220809/Input'

# DIRECTORY TO SAVE THE NEURO RESULTS
output_dir= '/share/fsmresfiles/mrh1996/4ceData_files_20220809'
################################################################################
...
# load icd package data and functions
load("icd/data/icd10_map_quan_deyo.rda")
load("icd/data/icd10_map_quan_elix.rda")

icd_funcs <- list.files(
  path = "icd/R",
  pattern = ".R",
  full.names = TRUE
)
for (func in icd_funcs) {
  source(func)
}
rm(icd_funcs, func)
